(() => {
  ...
  const MODULE_ID = "galaxy-map-framework";
  ...
  function notifyError(message) {
    ui.notifications?.error(`[Galaxy Map Framework] ${message}`);
  }

  function notifyInfo(message) {
    ui.notifications?.info(`[Galaxy Map Framework] ${message}`);
  }
  ...
  function renderCrudDialog({ title, content, submitLabel = "Save", onSubmit, render = activateCrudDialog }) {
    new Dialog({
      ...
      classes: ["galaxy-map-framework", "gmf-crud-dialog"],
      width: 560
    }).render(true);
  }
  ...
  function openPlayerMapChooser() {
    ...
    dialog = new Dialog({
      ...
      classes: ["galaxy-map-framework", "gmf-crud-dialog", "gmf-map-chooser-dialog"],
      width: 460
    });
    dialog.render(true);
    return dialog;
  ...
  function addSceneControlsButton(controls) {
    const toolName = "galaxy-map-framework-open";
  ...
  class GalaxyMapManager extends getApplicationBase() {
    static DEFAULT_OPTIONS = {
      ...
      classes: ["galaxy-map-framework", "gmf-manager-window"],
      window: {
  ...
  class GalaxyMapView extends getApplicationBase() {
    static DEFAULT_OPTIONS = {
      id: "galaxy-map-view",
      classes: ["galaxy-map-framework", "gmf-map-window"],
      window: {
        title: "Galaxy Map",
  ...
  Hooks.on("getSceneControlButtons", addSceneControlsButton);
