import {
  ...
  MeshPhongMaterial, OctahedronGeometry, PerspectiveCamera, Scene, SphereGeometry, SRGBColorSpace,
  TextureLoader, TorusGeometry, WebGLRenderer,
...
export function createPlanetRenderer(host: HTMLElement, options: any) {
  ...
  const material = new MeshPhongMaterial({
    color: 0xffffff,
    shininess: 4,
    specular: new Color(0x17212d),
    flatShading: shape === "asteroid" || shape === "crystal"
  });
  ...
  const draw = () => { if (renderer && visible()) renderer.render(scene, camera); };
  const schedule = () => {
    ...
    timer = setTimeout(() => {
      ...
      frame = requestAnimationFrame(time => {
        ...
        if (!dragging) planet.rotation.y += Math.min(lastTime ? (time - lastTime) / 1000 : 0, 0.1) * 0.13;
        lastTime = time;
        draw();
        schedule();
      });
